...
{* TRAC form header *}
  ...
  TListFormT = SortedList<string,TFormT>;
  {class(TStringList)
    destructor Destroy; override;
  end;}
 ...
 TProcList = SortedList<string,TProcTRAC>; 
...
{ letter to integer 2k5}
 ...
 {**************************************}
 function ch8l(lt : letter) : char;
{ letter to TRChar 2k5/13}
 begin
  if lt < 256 then ch8l := chr(lt)
  else ch8l := #0;
 end;
{**************************************}
...
{***************************************}
  ...
  function TTRAC.InsFormT(var f : TFormT; const s: Alpha) : boolean;
   ...
   begin
    str := TRtoString(s);
    Idx := ListForms.IndexOfKey(str);
    found := Idx>=0;
    InsFormT := found;
    if found then {* t-form already exists *}
     f := TFormT(ListForms.Values[Idx])
    else
    begin
     f := TFormT.Create;
     ListForms.Add(str,f);
     Idx := ListFun.IndexOfKey(str); 
     found := Idx>=0;
     if found then
     begin
      tp := TProcTRAC(ListFun.Values[Idx]);
      with tp do
...
{  (sc) ss, segment TRString  }
  var f: TFormT; s: Alpha; I,k: integer;
  begin
  with TT do
  begin
    Par(1,s);
   ...
   begin B:=nil;
    ...
    begin if I <> 0 then
     for J:=APar(1)+1 to APar(2)-1 do
      WrChL0(ChMem[J],B);
     s := StoTRString(ListForms.Keys[I]);
...
{  + (ww) oi, input file  }
   ...
   begin
    ...
    If FileExists(NM) then
     begin assign(FI,NM); reset(FI); LEN:=1 end
     else begin LEN:=-1; OPNFI:=false end;
...
{***************************************}
  ...
  begin
   ...
   Idx := ListFun.IndexOfKey(SFunName);
   if Idx  >= 0 then
    if Update then
    begin
     tp := TProcTRAC(ListFun.Values[Idx]);
     tp.flags := tp.flags + [fptClearStack,fptOverride];
     tp.t_proc := Fun
    end
    else Exit
   else
...
{***************************************}
  ...
  {***************************************}
   ...
   begin
     ...
     WrTrace('Processor TRAC 4Z (MACAR 2K5/13)',True,0);
     AlgTRAC;
     WrTrace('Processor read command STOP.',True,0);
     FinTRAC;
   end;
   {**************************************}
  ...
  {**************************************}
   ...
   begin
    ...
    if AStr[I]=#1 then
    begin
     Result := Result+TRChar(256*Ord(AStr[I+1])+Ord(AStr[I+2]));
     I := I+3;
    end
    else
    begin
     Result := Result+TRChar(AStr[I]);
     I := I+1;
    end
   end;
   {**************************************}
   ...
   begin
    Result := '';
    I := 1;
    for I:=1 to Length(ATRStr) do
    begin
     lt := lch(ATRStr[I]);
     if (lt <> 1) and (lt < 256) then
      Result := Result+ch8l(lt)
     else
      Result := Result+#1+ch8l(lt div 256)+ch8l(lt mod 256)
    end;
